...
const MenuButtonStyled = styled(MenuButton)`
    margin-right: ${({$ratio}) => $ratio * 16}px;
    ${({$isPart}) => $isPart ? 'z-index: 2' : ''};
`;
...
export const Game = ({ isFirstTimeRules, picture, bg, phrases, level, initialPuzzles, isAlreadyFinished}) => {
    const { next, setLastPosition, lastPosition } = useProgress();
    ...
    useEffect(() => {
        ...
        const order = isAlreadyFinished ? Array.from({length: 16}, (_, i) => i) : initialPuzzles;
        const initial = (lastPosition.length > 0 && !isAlreadyFinished) ? lastPosition : order.map((id) => fillCellArray(id, size, id === cellsAmount - 1, picture));
        setShownCells(() => initial);
        setInitialCells(() => initial);
    }, [ratio, picture, initialPuzzles, isAlreadyFinished, lastPosition]);
    ...
    const onMoveCell = useCallback((ind) => {
        ...
        if (typeof emptyId === 'number') {
            ...
            if (i >= SKIP_AMOUNT && !isAlreadyFinished) {
                setCanSkip(true);
            }
    ...
    }, [shownCells, setFinished, setShownCells, finished, next, initialCells, isAlreadyFinished, setLastPosition]);
        ...
        <>
            <Wrapper $bg={bg}>
                <ButtonsBlock $ratio={ratio}>
                    <FlexWrapper>
                        <MenuButtonStyled $isPart={menuRules} $ratio={ratio} ratio={ratio} onClick={handleOpenMenu} />
                        <ActionButton $ratio={ratio} onClick={() => setRulesModal({shown: true, isFirstTime: false})}>
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fillRule="evenodd" clipRule="evenodd" d="M12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22ZM12 7.75C11.3787 7.75 10.875 8.25368 10.875 8.875C10.875 9.28921 10.5392 9.625 10.125 9.625C9.71079 9.625 9.375 9.28921 9.375 8.875C9.375 7.42525 10.5503 6.25 12 6.25C13.4497 6.25 14.625 7.42525 14.625 8.875C14.625 9.58584 14.3415 10.232 13.883 10.704C13.7907 10.7989 13.7027 10.8869 13.6187 10.9708C13.4029 11.1864 13.2138 11.3753 13.0479 11.5885C12.8289 11.8699 12.75 12.0768 12.75 12.25V13C12.75 13.4142 12.4142 13.75 12 13.75C11.5858 13.75 11.25 13.4142 11.25 13V12.25C11.25 11.5948 11.555 11.0644 11.8642 10.6672C12.0929 10.3733 12.3804 10.0863 12.6138 9.85346C12.6842 9.78321 12.7496 9.71789 12.807 9.65877C13.0046 9.45543 13.125 9.18004 13.125 8.875C13.125 8.25368 12.6213 7.75 12 7.75ZM12 17C12.5523 17 13 16.5523 13 16C13 15.4477 12.5523 15 12 15C11.4477 15 11 15.4477 11 16C11 16.5523 11.4477 17 12 17Z" fill="white"/>
                            </svg>
                        </ActionButton>
                    </FlexWrapper>
                    <p>Уровень {level}/5</p>
